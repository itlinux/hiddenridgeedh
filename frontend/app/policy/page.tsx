...
import { ArrowLeft, MessageSquare, Bell, ShieldAlert, XCircle, Users, Info, PartyPopper, Briefcase, CheckCircle2, Lock } from 'lucide-react';
import PageHeader from '@/components/layout/PageHeader';

export default function PolicyPage() {
  return (
    <div className="min-h-screen bg-cream-50">
      <PageHeader label="Community" title="Site Policy" />
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        <div className="space-y-8">
            ...
            <div className="bg-red-50 border border-red-100 rounded-sm p-6 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <p className="font-body text-sm text-forest-700 leading-relaxed">
                  ...
                  may be removed to keep things focused.
                </p>
              </div>
              <div className="flex items-start gap-3">
                <XCircle size={16} className="text-red-400 flex-shrink-0 mt-0.5" />
          ...
          <div>
            <h2 className="font-serif text-xl text-forest-800 mb-4 flex items-center gap-2">
              <Lock size={18} className="text-forest-400" />
              Member data &amp; privacy
            </h2>
            <div className="bg-cream-100 border border-cream-200 rounded-sm p-6 space-y-3">
              <p className="font-body text-sm text-forest-600 leading-relaxed">
                All information shared on this site — names, addresses, phone numbers, photos,
                and anything in your profile — is <strong>strictly private to Hidden Ridge members</strong>.
              </p>
              <p className="font-body text-sm text-forest-600 leading-relaxed">
                By using this site you agree <strong>not to</strong>:
              </p>
              <ul className="space-y-2">
                {[
                  'Share or distribute any member information to people outside the neighborhood.',
                  'Use member contact details for solicitation, marketing, or commercial purposes.',
                  'Screenshot or export the member directory, map, or any personal data.',
                ].map((item, i) => (
                  <li key={i} className="flex items-start gap-2 font-body text-sm text-forest-600">
                    <XCircle size={14} className="text-red-400 flex-shrink-0 mt-0.5" />
                    {item}
                  </li>
                ))}
              </ul>
              <p className="font-body text-sm text-forest-600 leading-relaxed pt-2">
                Violations may result in immediate account suspension.
                See our full <Link href="/privacy" className="text-gold-500 hover:text-gold-600 underline">Privacy Policy</Link> for details.
              </p>
            </div>
          </div>
